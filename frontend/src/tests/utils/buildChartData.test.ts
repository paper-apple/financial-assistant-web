...
//       { count: 3, proportion: 20 },
//       { count: 2, proportion: 20 },
//     ]);
//   });
...
//     expect(result.datasets[0].meta[0].proportion).toBe(25);
//     expect(result.datasets[0].meta[1].proportion).toBe(25);
//   });

//   it("avg = 0, если count = 0", () => {
//     const rows = [{ key: "A", total: 100, count: 0 }];
//     const result = buildChartData(rows, 100);
//     expect(result.datasets[0].meta[0].proportion).toBe(0);
//   });
// });
...
describe("buildChartData", () => {
  ...
  it("корректно считает proportion", () => {
    const rows = [
      { key: "A", total: 100, count: 4 }, // 100/150 = 66.67%
      { key: "B", total: 50, count: 2 },  // 50/150 = 33.33%
    ];
    const result = buildChartData(rows, 150);
    expect(result.datasets[0].meta[0].proportion).toBeCloseTo(66.67, 2);
    expect(result.datasets[0].meta[1].proportion).toBeCloseTo(33.33, 2);
  });

  it("proportion = 100, если одна категория и count = 0", () => {
    const rows = [{ key: "A", total: 100, count: 0 }];
    const result = buildChartData(rows, 100);
    expect(result.datasets[0].meta[0].proportion).toBe(100);
    expect(result.datasets[0].meta[0].count).toBe(0);
  });
});
