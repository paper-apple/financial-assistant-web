  ...
  return (
    ...
    >
      {!confirmDelete ? (
        <div 
          key="normal" 
          className={`flex items-center w-full py-2 px-2 border-x bg-(--bg-secondary) border-(--top-bar-border) rounded-b-lg border-b
          `}
          style={{
        ...
        >
          <span // Отображение количества выбранных записей
            className="hidden sm:block label-text w-32.5 text-left items-start justify-start l-0 shrink-0 whitespace-nowrap mr-4">
        ...
        <div key="confirm"
          className={`flex justify-between items-center w-full py-2 px-2 border-x bg-(--bg-secondary) border-(--top-bar-border) rounded-b-lg border-b
          `}
          style={{
            marginLeft: !isSafari && hasScrollbar ? `${scrollbarWidth}px` : ''
          }}
        >
          <span className="label-text shrink-0 w-32.5 whitespace-nowrap mr-4">
