      ...
      {filters.length > 0 && (
        <>
          <div className="border-t pt-4 mb-4">
            <h3 className="font-medium mb-2">Текущие фильтры:</h3>
            <ul className="space-y-1">
              {filters.map((f, i) => (
                <li key={i} className="flex justify-between items-center">
                  <span className="text-sm">
                    {f.keyword && `"${f.keyword}" `}
                    {f.startDate && `с ${f.startDate.toLocaleString()} `}
                    {f.endDate && `до ${f.endDate.toLocaleString()} `}
                    {f.minPrice != null && `от ${f.minPrice} `}
                    {f.maxPrice != null && `до ${f.maxPrice}`}
                  </span>
                  <button
